...
# PART 3 — More Questions From Reviewer Logs (New Only)
*(Pulled from a separate batch of interviewer notes across many reviewers. Only genuinely new items not already in Part 1 or Part 2 are listed below.)*

---

## 🟥 JAVASCRIPT — Part 3 Additions

### High Priority
- **[T]** Basic terminology precision — identifier vs literal, primitive data types, escape sequences, `var`/`let`/`const` differences (asked as a standalone "explain the difference" more often than expected)
- **[T]** Promise.race, **Promise.all vs Promise.allSettled** (distinct from `.any`/`.race` pairing you already have)
- **[T]** Constructor deep-dive — copy constructor, types of constructors, *why do we need constructors at all*
- **[T]** Function borrowing (`call`/`apply`/`bind` framed specifically as "function borrowing")
- **[T]** `structuredClone` (as its own topic, beyond shallow/deep copy)
- **[T]** Switch vs if-else ladder, when to use `for` vs `while`, `map` vs `forEach` (given as standalone comparison questions)

### Medium Priority
- **[T]** Type assertion, generics (`generic keyword`, generic functions) — TypeScript-adjacent but repeatedly asked
- **[T]** Decorators
- **[T]** Object literals, Octal literals
- **[T]** `console.log(1 + +'1')` style coercion trick questions — practice reading unary `+` and implicit coercion snippets
- **[T]** ES6 coding standards (style/convention expectations, not just feature list)

### Practical
- **[P]** Find the key with the highest value in an object
- **[P]** Remove/find prime numbers from an array (sum of primes)
- **[P]** Move all zeroes in an array to the right/end with minimal time complexity — `[1,3,0,5,0,5,0] → [1,3,5,5,0,0,0]`
- **[P]** Binary-search-style "insert position" problem: given a sorted array and a target, return its index if present, else the index where it *would* be inserted
  - `[2,4,5,7,8], target=7 → 3` | `[2,4,5,7,8], target=3 → 1`
- **[P]** Find the sum of values in a **deeply nested** array of objects (arbitrary nesting depth), e.g. `[{af:[34,343]}, {af:[34,343]}, ...]`
- **[P]** Generator function: yield even numbers in reverse from 100 → 1, and report how many were yielded within a 300ms window
- **[P]** Create a Promise that checks if a target exists in an array via `.includes()`; resolve if found, then write the result to a file with `fs`
- **[P]** Linked-list digit problem: build two linked lists from objects, sum their last node values, reverse that sum, sum the digits of the result, then rebuild a linked list with each digit as a node (multi-step — good one to practice end-to-end)

---

## 🟧 NODE.JS / EXPRESS — Part 3 Additions

### High Priority
- **[T]** Libuv (asked repeatedly as a standalone deep-dive, not just "what handles the event loop")
- **[T]** Duplex stream vs Transform stream (distinct — several logs flagged "needs clarity" here)
- **[T]** `express.urlencoded()`, `express.json()`, `express.static()` — what each does and when needed
- **[T]** JWT — full structure, verification code (not just "what is JWT"), JWT components
- **[T]** Morgan — why use a logging *library* instead of writing your own logging function (asked "answer deep")
- **[T]** `process.nextTick()` specifically (beyond nextTick vs setImmediate comparison)
- **[T]** OPTIONS method, Content negotiation (as dedicated topics, not just under CORS)

### Medium Priority
- **[T]** TCP vs UDP
- **[T]** Thread pool (as distinct from worker threads / cluster)
- **[T]** `app.locals`, `app.all`
- **[T]** Partials, template engines (view engines beyond EJS)
- **[T]** Handling cookie expiry
- **[T]** "Page not found" (404) middleware — how to implement
- **[T]** `exec` vs `execFile` (distinct pairing from fork/spawn)

---

## 🟩 REACT — Part 3 Additions

### High Priority
- **[T]** Babel vs Webpack — asked as its own comparison question repeatedly
- **[T]** Higher-Order Components — **with worked examples**, not just definition
- **[T]** Error Boundary — implementation code, not just concept
- **[T]** `useState` and `useReducer` — asked as standalone deep-dives (return values, update patterns)
- **[T]** Purpose of Custom Hooks — with concrete examples
- **[T]** `PropTypes` (basic prop validation, separate from PropTypes vs TypeScript comparison)
- **[T]** `React.Fragment`, `React.memo`, React Fiber — each asked standalone
- **[T]** Redux **"time travel" debugging** — what it is, how Redux DevTools enables it

### Medium Priority
- **[T]** Dead code elimination, Polyfills, Event pooling (bundler/runtime-adjacent trio, often asked together)
- **[T]** `forwardRef`, Referential equality (asked as standalone rapid-fire items)
- **[P]** Create a "props proxy" pattern for a HOC (specific implementation exercise, asked multiple times)
- **[T]** Rules for hooks/custom hooks, Controlled vs Uncontrolled components (as standalone rapid-fire, distinct phrasing from Part 1)

---

## 🟨 MONGODB — Part 3 Additions

### High Priority
- **[T]** `$bulkWrite`, `$facet`, `$lookup` — asked as standalone deep-dives very frequently in this batch
- **[T]** Journaling (as its own topic, not just under replication)
- **[T]** Replication (standalone, beyond the architecture deep-dive in Part 2)
- **[T]** `mongodump` / backup tooling
- **[T]** CAP theorem, Hashed index, Replica set (each asked standalone)

### Practical
- **[P]** Create a compound index / drop an index / check if a collection is capped / create a capped collection (asked as a connected practical sequence — practice them together)
- **[P]** Find the first three names of highest-salary people
- **[P]** Country-wise people count, average age, and highest salary (multi-metric aggregation in one pipeline)
- **[T]** `$addFields`, `$addToSet`, `$elemMatch` (standalone, beyond the operator list in Part 2)

---

## 🟦 DSA — Part 3 Additions

### High Priority
- **[T]** Heap Sort — algorithm walkthrough (asked standalone very often)
- **[T]** Merge Sort vs Quick Sort — direct comparison question (complexity/stability tradeoffs)
- **[T]** Time complexity of Binary Search, Quick Sort (best/avg/worst), Bubble Sort, and BST insertion — expect to be asked to state these precisely, not just "log n" vaguely
- **[T]** Rehashing, Double hashing (flagged "needs clarity" in multiple logs — worth extra prep)
- **[T]** Degree of a tree, Spanning tree (as standalone, distinct from "minimum spanning tree")
- **[T]** Applications of Hash Table and Stack (combined framing)

### Practical
- **[P]** Check for valid/balanced parentheses
- **[P]** Create a singly linked list; write a **recursive** method to print it in reverse
- **[P]** Merge two linked lists
- **[P]** Remove duplicate elements from a linked list (given starter code)
- **[P]** Remove nodes with odd values from a linked list
- **[P]** Create a sorted array from a BST (in-order traversal application)
- **[P]** Convert a string to Title Case

### Medium Priority
- **[T]** A* algorithm (pathfinding — distinct from Dijkstra's/Bellman-Ford already covered)
- **[T]** Downsides of dynamic memory allocation, sparse arrays, B-trees, UTF-8, grapheme (character-encoding cluster of questions)
- **[T]** Hierarchical Linked List, Full Tree, Perfect Tree (as standalone definitions)
- **[T]** Graph indexing
